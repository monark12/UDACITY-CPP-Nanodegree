...
Renderer::Renderer(const std::size_t screen_width,
  ...
  // Load font
  sdl_font = TTF_OpenFont("/usr/share/fonts/truetype/freefont/FreeSans.ttf", 50);
  if(nullptr == sdl_font) {
...
void Renderer::DrawCircle(SDL_Renderer * renderer, int centreX, int centreY, int radius)
{
...
void Renderer::Render(Snake const snake, SDL_Point const &food) {
  ...
  DrawCircle(sdl_renderer, block.x+(block.w/2), block.y+(block.h/2), block.h/2);
...
int Renderer::RenderStartMenu(std::string str) {
  ...
  bool selected[number_of_menu_items] = {0, 0};
  SDL_Color White = {255, 255, 255, 0};
  SDL_Color Red = {255, 0, 0, 0};
  SDL_Surface *screen = SDL_GetWindowSurface(sdl_window);

  surface_menus[0] = TTF_RenderText_Solid(sdl_font, text_labels[0], White);
